...
const ListComponent = ({ route, navigation }) => {
  ...
  //filter my task data which have name  Test Drive Approval to hide for DSE role
  const myTaskDSEData = myTasksData?.filter(
    (data) => data.taskName !== "Test Drive Approval"
  );
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.LIGHT_GRAY, padding: 5 }}>
      ...
      {index === 0 && myTasksData.length > 0 && (
        <FlatList
          data={homeSelector.isDSE ? myTaskDSEData : myTasksData}
          extraData={homeSelector.isDSE ? myTaskDSEData : myTasksData}
          style={{ flex: 1 }}
          numColumns={3}
