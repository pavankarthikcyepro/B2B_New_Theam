...
const EvaluationFollowUpScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    navigation.addListener("focus", () => {
      ...
      if (taskName === "Contacts followup") {
        // this change is to send the previously used taskName value to the service call.
  ...
  const getReasonListData = async (taskName) => {
    ...
    if (employeeData) {
      ...
      Promise.all([dispatch(getReasonList(payload))])
        .then((res) => {
          let tempReasonList = [];
  ...
  const changeTaskStatusBasedOnActionType = (type) => {
    ...
    newTaskObj.reason =
      selector.reason === "Other" ? otherReason : selector.reason;
