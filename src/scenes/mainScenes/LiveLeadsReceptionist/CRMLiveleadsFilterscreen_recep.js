...
import { updateDealerFilterData, updateFilterSelectedData, saveFilterPayload, updateFilterLevelSelectedData, updateLiveLeadObjectData, } from "../../../redux/liveLeadsReducerReceptionist"
import { useIsFocused } from "@react-navigation/native";
import AnimLoaderComp from "../../../components/AnimLoaderComp";
import { Colors } from "../../../styles";
import { updateLiveLeadObjectDataCRM, getCRMEmployeesDropDownDataReception } from "../../../redux/liveLeadsReducerReceptionist";
...
const CRMLiveleadsFilterscreen_recep = ({ route, navigation }) => {
      ...
      localData?.length &&
        localData.forEach((val) => {
          if (val.selected !== undefined && val.selected === true)
            selectedParentIds.push(val.designation);
        });
      if (selectedParentIds?.length > 0) {
        let data = dropdownDatas.filter((item) =>
          selectedParentIds.includes(item.designation)
        );
        dropdownDatas = [...data];
      }
    }
  ...
  const updateSelectedItemsForEmployeeDropDown = (data, index, index1) => {
    ...
    if (index === 0) {
      ...
      let key2 = employeeTitleNameList[index + 1];

      // const newTotalDataObjLocal = { ...employeeDropDownDataLocal };
      const newTotalDataObjLocal2 = Object.assign(employeeDropDownDataLocal);
      let objIndex2 = newTotalDataObjLocal2[key2].findIndex(
        (obj) => obj.name == data.name
      );
      const a2 = newTotalDataObjLocal2[key2].map((data, index) =>
        index === objIndex2
          ? { ...newTotalDataObjLocal2[key2][index], selected: true }
          : { ...newTotalDataObjLocal2[key2][index], selected: false }
      );
      newTotalDataObjLocal[key2] = a2;
      let arrayData2 = [];
      for (let key in employeeDropDownDataLocal) {
        arrayData = employeeDropDownDataLocal[key].map((element, i) => {

          element.name === data.designation ? element.selected = true : null;
      ...
      for (let key in employeeDropDownDataLocal) {
        arrayData = employeeDropDownDataLocal[key].map((element, i) => {
          ...
          element.name === data.name ? element.selected = true : null;
  ...
  const getDashboadTableDataFromServer = async (selectedIds, from, selectedBranchName = "") => {
    ...
    if (!selector.isDSE){
      Promise.all([dispatch(getCRMEmployeesDropDownDataReception(payload1))])
        .then(() => {
          // Promise.all([
  ...
  useEffect(() => {
    if (selector.crm_employees_drop_down_data_recepLiveleads) {
      let names = [];
      let newDataObj = {};
      for (let key in selector.crm_employees_drop_down_data_recepLiveleads) {
        const arrayData = selector.crm_employees_drop_down_data_recepLiveleads[key];
        if (arrayData.length != 0) {
          names.push(key);
        }
        const newArray = [];
  ...
  }, [selector.crm_employees_drop_down_data_recepLiveleads]);
...
export default CRMLiveleadsFilterscreen_recep;
