...
const BookingFormScreen = ({ route, navigation }) => {
  ...
  const [showBookingPaymentSection, setShowBookingPaymentSection] =
    useState(false);
  ...
  const getPreBookingDetailsFromServer = () => {
    if (universalId) {
      dispatch(getbookingDetailsApi(universalId));
    }
  };
  ...
  useEffect(() => {
    if (selector.pre_booking_details_response) {
      ...
      if (dmsLeadDto.leadStatus === "BOOKINGCOMPLETED") {
        setShowBookingPaymentSection(true);
        // Get Payment Details
  ...
  const getBookingListFromServer = async () => {
    if (userData.employeeId) {
      let endUrl = "?limit=10&offset=" + "0" + "&status=BOOKING&empId=" + empId;
      dispatch(getBookingData(endUrl));
    }
  };
  ...
  const approveOrRejectMethod = (type) => {
    ...
    if (type === "APPROVE") {
      dmsLeadDto.leadStatus = "BOOKINGCOMPLETED";
    } else if (type === "REJECT") {
