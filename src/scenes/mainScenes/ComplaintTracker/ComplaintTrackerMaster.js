...
import { getCountsComplaintsDashboard, getEmpComplaintDashboard, getEmpComplaintMaster, setCheckboxStatus } from '../../../redux/complaintTrackerReducer';
import Pressable from 'react-native/Libraries/Components/Pressable/Pressable';
import { Checkbox, IconButton } from 'react-native-paper';
import { RadioTextItem, RadioTextItem2 } from '../../../pureComponents';
...
const ComplaintTrackerMaster = ({ route, navigation }) => {
    ...
    const getUserData = async () => {
        try {
            ...
            if (employeeData) {
                ...
                let payload = {
                    empId: dataObject?.employeeId
                }
                // dispatch(getCountsComplaintsDashboard(payload));
                dispatch(getEmpComplaintMaster(payload));
            }
        } catch (error) {
            alert(error);
        }
    ...
    const eventListTableRow = useCallback(
        (
            empName, AtiveCount, ClosedCount, isViewDetailsVisible = false,isBorder=false
        ) => {
            return (<>
                <View style={{
                    flexDirection: "row",
                    ...
                    <View style={{ flex: 1 }}>
                        <Text style={[styles.TitleTabelText,{padding:5}]}>{empName}</Text>
                    </View>
                    <View style={{ flexDirection: "row", flex: 1 }}>
                        <Pressable onPress={()=>{
                            ...
                            <Text style={{
                                ...
                                marginEnd: 10,
                                // flex: 1,
                                borderColor: random_color("hex"),
                                borderWidth: isBorder ? 1 : 0,
                                borderRadius: 10,
                                textDecorationLine: isBorder ? "underline" : "none",
                                width: 50,
                                padding: 5
                            }}>{AtiveCount}</Text>
                        </Pressable>
    ...
    return (
        <SafeAreaView style={styles.conatiner}>
            <View style={{ padding: 10, }}>
                ...
                >
                 <Text>Complaint Master</Text>
                 <View style={{flexDirection:"row"}}>
                        <RadioTextItem2
                            label={"Stage wise"}
                            value={"Stage wise"}
                            disabled={false}
                            status={selector.stage_wise_checkbox ? true : false}
                            onPress={() => {
                                dispatch(
                                    setCheckboxStatus({
                                        key: "STAGE_WISE_CHECKBOX",
                                        text: "",
                                    })
                                )
                        ...
                        />
                        <RadioTextItem2
                            label={"Complaint Facotor"}
                            value={"Complaint Facotor"}
                            disabled={false}
                            status={selector.complaintFacoty_wise_checkbox ? true : false}
                            onPress={() => {
                                dispatch(
                                    setCheckboxStatus({
                                        key: "COMPLAINT_FACTOR_CHECKBOX",
                                        text: "",
                                    })
                                )
                        ...
                        <RadioTextItem2
                            ...
                            value={"Department"}
                            disabled={false}
                            status={selector.department_wise_checkbox ? true : false}
                            onPress={() => {
                               
                                dispatch(
                                    setCheckboxStatus({
                                        key: "DEPARTMENT_CHECKBOX",
                                        text: "",
                                    })
                                )
                            }}
                 ...
                 <View style={{justifyContent:"center",padding:10}}>
                        ...
                        {selector.stage_wise_checkbox ?
                            <FlatList
                                data={selector.complaintTrackerDashboardData_Master?.stageWise}
                                bounces={false}
                                    ...
                                    return (
                                        <>
                                            <View style={{ marginVertical: 10 }}>
                                                ...
                                                {eventListTableRow(item.reportValue, item.activeCount, item.closedCount, false, true)}
                        ...
                        {selector.complaintFacoty_wise_checkbox ?
                            <FlatList
                                data={selector.complaintTrackerDashboardData_Master?.complaintFactorWise}
                                bounces={false}
                                renderItem={({ item, index }) => {
                        ...
                        {selector.department_wise_checkbox ?
                            <FlatList
                                data={selector.complaintTrackerDashboardData_Master?.departmentWise}
                                bounces={false}
                                renderItem={({ item, index }) => {
...
export default ComplaintTrackerMaster
