...
import {  updateDealerFilterData, updateReceptionistFilterids } from "../../../redux/complaintTrackerReducer";
...
const ComplaintTrackerBasicFilter = ({ route, navigation }) => {
    ...
    useEffect(() => {
        if (nameKeyList.length > 0 && isFocused && selector.ComplaintTrackerselector?.levelSelected?.length > 4) {
    ...
    const dropDownItemClicked = async (index, initalCall = false) => {
        ...
        if (index === 4) {
            ...
            if (initalCall) {
                if (initalCall) {
                    let levelIds = selector.ComplaintTrackerselector?.levelSelected;
...
export default ComplaintTrackerBasicFilter;
