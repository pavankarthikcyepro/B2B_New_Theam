...
import { updateDealerFilterData, updateFilterSelectedData, updateFilterLevelSelectedData, updateLiveLeadObjectData, } from "../../../redux/complaintTrackerReducer"
...
const CRMFilterscreenComplaintTracker = ({ route, navigation }) => {
  const selector = useSelector((state) => state.homeReducer);
  const ComplaintTrackerselector = useSelector((state) => state.complaintTrackerReducer);
  ...
  useEffect(() => {
    navigation.addListener("focus", () => {
      if (!isEmpty(ComplaintTrackerselector.dealerFilter)) {
        const temp = { ...ComplaintTrackerselector.dealerFilter };
  ...
  const setName = useCallback(
    (names, newDataObj) => {
      ...
      if (!isEmpty(names) && !isEmpty(newDataObj)) {
        setEmloyeeTitleNameList(names);
        if (!isEmpty(ComplaintTrackerselector.filterSelectedData)) {
          const temp = { ...ComplaintTrackerselector.filterSelectedData };
          setEmployeeDropDownDataLocal(temp);
        } else {
  ...
  const submitBtnForEmployeeData = () => {
    ...
    if (!_.isEmpty(ComplaintTrackerselector.dealerFilter)) {
      let selectedIds = [];
        ...
        if (arrayData.length != 0) {
          arrayData.forEach((element) => {
            if (element.selected === true) {
              if (key == "Select Designation"){
                selectedDesignations.push(element.name);
              }else{
                selectedEmpNAme.push(element.name);
              }
      ...
      if (temp.length > 0) {
        ...
        let tempPayload = {
          ...
          dealerCodes: selectedBranchName,
          selectedDesignation:selectedDesignations,
...
export default CRMFilterscreenComplaintTracker;
