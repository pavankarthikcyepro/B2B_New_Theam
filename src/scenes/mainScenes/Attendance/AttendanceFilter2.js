...
const CurrentMonthFirstDate = moment(currentDateMain, dateFormat)
  .subtract(0, "months")
  .startOf("month")
  .format(dateFormat);
...
const FilterScreen2 = ({ route, navigation }) => {
        ...
        if (dataArray.length > 0) {
          const newDataArry = dataArray.map((subItem, index) => {
            ...
            if (selectedParendIds2.includes(Number(obj.parentId))) {
              ...
              if (key === "Dealer Code") {
                // to restrict only users assigned branches selection and auto populate
  ...
  const submitBtnClicked = async (initialData = null) => {
    const temp = totalDataObj[nameKeyList[nameKeyList.length - 1]].sublevels;
    const check = temp.filter((i) => i.selected);
    ...
    if (check.length > 0) {
       const selectedIds = [];
       for (i =0; i < nameKeyList.length; i++) {
    ...
    if (!_.isEmpty(storeDropDownClickdata)) {
      let temp = await updateSelectedItemsSubmit(
        storeDropDownClickdata,
        storeDropDownClickIndex
      );
      if (temp) {
    ...
    if (selector.isDSE && check.length > 0) {
  ...
  const getDashboadTableDataFromServer = (
    ...
    Promise.all([
      ...
      .then(() => {
        if (from == "EMPLOYEE") {
          dispatch(
            updateEmpDropDown_Local(selector.employees_drop_down_data_local)
          );
  ...
  useEffect(() => {
    if (selector.employees_drop_down_data_local) {
      let filterIds = selector.filterIds?.allEmpSelected || [];
      let names = [];
      let newDataObj = {};
      for (let key in selector.employees_drop_down_data_local) {
        names.push(key);
        const arrayData = selector.employees_drop_down_data_local[key];
        const newArray = [];
        if (arrayData.length > 0) {
          arrayData.forEach((element) => {
            newArray.push({
  ...
  }, [selector.employees_drop_down_data_local]);
      ...
      <DropDown
        ...
        selectedItems={(item) => {
          if (dropDownFrom === "ORG_TABLE") {
            ...
            // let tempTrunintoArrary = [item].map(item => item.selected = true);
