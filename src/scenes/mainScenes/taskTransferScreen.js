...
import {
  ...
  getEmployeeDetails,
  getDeptDropdownTrans,
  getDesignationDropdownTrans,
  getEmployeeDetailsTrans,
  updateStatus,
  submitTaskTransfer,
} from "../../redux/taskTransferReducer";
...
const TaskTranferScreen = () => {
  ...
  const [showTrasnferFromDropdowns, setShowTrasnferFromDropdowns] =
    useState(false);
  ...
  const [branchTransferFromDropDownItem, setbranchTransferFromDropDownItem] =
    useState("");
  const [branchTransferFromDropDownList, setbranchTransferFromDropDownList] =
    useState([]);
  const [deptTransferFromDropDownItem, setDeptTransferFromDropDownItem] =
    useState("");
  const [deptTransferFromDropDownList, setDeptTransferFromDropDownList] =
    useState([]);
  const [
    designationTransferFromDropDownItem,
    setDesignationTransferFromDropDownItem,
  ] = useState("");
  const [
    designationTransferFromDropDownList,
    setDesignationTransferFromDropDownList,
  ] = useState([]);
  const [
    employeeTransferFromDropDownItem,
    setEmployeeTransferFromDropDownItem,
  ] = useState(null);
  const [
    employeeTransferFromDropDownList,
  ...
  useEffect(() => {
    if (selector.branchList.length > 0) {
      setbranchDropDownList(
        selector.branchList.map(({ id: value, value: label, ...rest }) => ({
          value,
          label,
          ...rest,
        }))
      );
      setbranchTransferFromDropDownList(
        selector.branchList.map(({ id: value, value: label, ...rest }) => ({
          value,
          label,
          ...rest,
  ...
  useEffect(() => {
    if (selector.deptList.length > 0) {
      setDeptDropDownList(
        selector.deptList.map(({ id: value, value: label, ...rest }) => ({
          value,
          label,
          ...rest,
        }))
  ...
  useEffect(() => {
    if (selector.deptListTrans.length > 0) {
      setDeptTransferFromDropDownList(
        selector.deptListTrans.map(({ id: value, value: label, ...rest }) => ({
          value,
          label,
          ...rest,
        }))
  ...
  useEffect(() => {
    if (selector.employeeList.length > 0) {
      setEmployeeDropDownList(
        selector.employeeList.map(
          ({ empId: value, empName: label, ...rest }) => ({
            value,
            label,
            ...rest,
          })
  ...
  useEffect(() => {
    if (selector.employeeListTrans.length > 0) {
      setEmployeeTransferFromDropDownList(
        selector.employeeListTrans.map(
          ({ empId: value, empName: label, ...rest }) => ({
            value,
            label,
            ...rest,
  ...
  useEffect(() => {
    if (selector.designationList.length > 0) {
      setDesignationDropDownList(
        selector.designationList.map(
          ({ id: value, value: label, ...rest }) => ({ value, label, ...rest })
        )
      );
    }
  ...
  useEffect(() => {
    if (selector.designationListTrans.length > 0) {
      setDesignationTransferFromDropDownList(
        selector.designationListTrans.map(
          ({ id: value, value: label, ...rest }) => ({ value, label, ...rest })
        )
      );
    }
  ...
  useEffect(() => {
    if (selector.taskList.length > 0) {
      ...
      for (let i = 0; i < selector.taskList.length; i++) {
        let index = tempTaksList.findIndex(
          (item) => item.name === selector.taskList[i].taskName
        );
        if (index === -1) {
          tempTaksList.push({
  ...
  useEffect(() => {
    setDeptDropDownItem("");
    setDesignationDropDownItem("");
    setEmployeeDropDownItem("");
    clearDataOnChangeDropDown();
  }, [branchDropDownItem]);
  ...
  const clearDataOnChangeDropDown = () => {
    setShowTrasnferFromDropdowns(false);
    setShowTaskList(false);
    setShowTaskFlatlist(false);
    setTasklistHeader(false);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        ...
        >
          ...
          {showTaskList ? (
            <View style={{ paddingVertical: 18, maxHeight: 350 }}>
              <FlatList
                data={taskList}
            ...
            >
              <View style={{ width: 800 }}>
                <FlatList
                  ...
                  extraData={checked}
                  renderItem={({ item, index }) =>
                    renderItemTaskTransferList(item, index)
                  }
                />
              </View>
            </ScrollView>
