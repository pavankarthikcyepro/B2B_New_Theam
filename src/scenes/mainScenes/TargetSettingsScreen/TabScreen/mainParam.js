        ...
        if (selectedBranch === null) {
            ...
            if (employeeData) {
                ...
                let payload = {
                    "branch": selectedBranch.value,
                    // "branchmangerId": otherDropDownSelectedValue.filter((item) => item.key === 'Managers').length > 0 ? otherDropDownSelectedValue.filter((item) => item.key === 'Managers')[0].value.value : '',
                    "employeeId": loggedInEmpDetails?.employeeId,
                    "endDate": selector.endDate,
                    // "managerId": otherDropDownSelectedValue.filter((item) => item.key === 'Managers').length > 0 ? otherDropDownSelectedValue.filter((item) => item.key === 'Managers')[0].value.value : '',
                    "retailTarget": retail,
                    "startDate": selector.startDate,
    ...
    function RenderTeamsSelfData(type) {
        return (
            <TextInput editable={editParameters} style={editParameters ? styles.textBox : styles.textBoxDisabled}
                       value={`${updatedSelfParameters[type]}`} keyboardType={'number-pad'}
            ...
            return (
                <>
                    {Number(item.employeeId) !== Number(loggedInEmpDetails?.empId) && selector.endDate === item.endDate && selector.startDate === item.startDate &&
                        <View>
                            <TextInput editable={editParameters}
            ...
            <>
                ...
                {loggedInEmpDetails !== null && (homeSelector.isTeamPresent && selector.isTeam) &&
                    <View style={{flexDirection: 'row'}}>
                        ...
                        <ScrollView style={{width: '100%'}} contentContainerStyle={{flexDirection: 'column'}}
                                            ...
                                            <>
                                                {Number(item.employeeId) !== Number(loggedInEmpDetails?.empId) && selector.endDate === item.endDate && selector.startDate === item.startDate &&
                                                    <TouchableOpacity style={styles.textBox} onPress={() => {
                                                        ...
                                                        <Text
                                                            style={styles.textInput}>{item.retailTarget !== null && selector.endDate === item.endDate && selector.startDate === item.startDate ? item.retailTarget : 0}</Text>
                                                    </TouchableOpacity>
                                                }
                                            </>
                                        )
                ...
                {/*Serlf Data Section*/}
                {ownData !== null && loggedInEmpDetails !== null && (homeSelector.isTeamPresent && !selector.isTeam) &&
                    <View style={{flexDirection: 'row'}}>
                        <View style={{width: '30%',}}>
                            <View style={{height: 35,}}></View>
                            <View style={styles.paramBox}>
                        ...
                        <ScrollView style={{width: '100%'}} contentContainerStyle={{flexDirection: 'column'}}
                            ...
                            <View style={styles.textBoxWrap}>
                                <TouchableOpacity style={styles.textBox} onPress={() => {
                                    console.log('678876545: ', loggedInEmpDetails)
                                    ...
                                    <Text
                                        style={styles.textInput}>{ownData.retailTarget !== null && selector.endDate === ownData.endDate && selector.startDate === ownData.startDate ? ownData.retailTarget : 0}</Text>
                                </TouchableOpacity>
                            </View>
